...
import AuthPage from '../components/Auth/AuthPage'; // Importar AuthPage
...
function HomePage({ openAuthModal }) { // Recibir openAuthModal como prop
  ...
  useEffect(() => {
    ...
    const fetchCourts = async () => {
      try {
        setLoading(true);
        const response = await api.get('/courts/'); // Realizar solicitud GET a /api/courts/
        setCourts(response.data); // Guardar la lista de canchas en el estado
        setLoading(false);
      } catch (err) {
        setError(err);
        setLoading(false);
  ...
  return (
    <> {/* Usar Fragmento para poder pasar props al Layout */}
      {/* El Layout se renderiza en App.js y envuelve HomePage.
          Necesitamos pasar openAuthModal a través del Layout al Header.
          Esto requiere modificar Layout.js para aceptar y pasar la prop. */}
      ...
      {/* showAuthModal && (
          <div className="modal-overlay"> {/* Reutilizar clase de overlay del modal de imagen */}
              <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '400px', padding: '2rem' }}> {/* Reutilizar clase de contenido, ajustar tamaño */}
                  <button className="modal-close-btn">✕</button> {/* Reutilizar clase de botón cerrar */}
                  <AuthPage /> {/* Renderizar el contenido de la página de autenticación */}
              </div>
          </div>
    </>
  );
